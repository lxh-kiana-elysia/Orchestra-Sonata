...
> 文档版本：v1.3（2026-09-12）
> 依据：`GDD.docx` v1.11（装备随少女绑定、员工死亡掉落）、`FDD.md` v2.8（FDD-11 装备系统）
...
> ⚠️ 顶层用 `{ "equipments": [ ... ] }` 包裹——`JsonUtility` **不支持顶层数组**（FDD §0 通用约定）。
...
| 日期 | 版本 | 说明 |
|---|---|---|
| 2026-09-12 | v1.1 | 质检修复：依据升为 GDD v1.9 / FDD v2.6 |
| 2026-09-12 | v1.3 | **修正 JSON 顶层结构**：`Equipments.json` 示例由裸数组改为 `{ "equipments": [ ... ] }`——JsonUtility **不支持顶层数组**（FDD §0 通用约定） |
| 2026-09-12 | v1.2 | **纠正误判**：丸山彩的专属装备原被标为"待异想体确定后补"并暗示阻塞 → 现明确为**待补、不阻塞**（GDD §3.2：异想体绑定为可选彩蛋，异想体与角色不要求一一对应，装备缺失不影响任何任务与剧情）。依据升 GDD v1.10 |
